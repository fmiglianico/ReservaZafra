<?php

if(!$_POST) exit;

// Email address that messages will be sent to;
$address = "reservazafra@gmail.com";

// Email address verification, do not edit.
function isEmail($email) {
	return(preg_match("/^[-_.[:alnum:]]+@((([[:alnum:]]|[[:alnum:]][[:alnum:]-]*[[:alnum:]])\.)+(ad|ae|aero|af|ag|ai|al|am|an|ao|aq|ar|arpa|as|at|au|aw|az|ba|bb|bd|be|bf|bg|bh|bi|biz|bj|bm|bn|bo|br|bs|bt|bv|bw|by|bz|ca|cc|cd|cf|cg|ch|ci|ck|cl|cm|cn|co|com|coop|cr|cs|cu|cv|cx|cy|cz|de|dj|dk|dm|do|dz|ec|edu|ee|eg|eh|er|es|et|eu|fi|fj|fk|fm|fo|fr|ga|gb|gd|ge|gf|gh|gi|gl|gm|gn|gov|gp|gq|gr|gs|gt|gu|gw|gy|hk|hm|hn|hr|ht|hu|id|ie|il|in|info|int|io|iq|ir|is|it|jm|jo|jp|ke|kg|kh|ki|km|kn|kp|kr|kw|ky|kz|la|lb|lc|li|lk|lr|ls|lt|lu|lv|ly|ma|mc|md|me|mg|mh|mil|mk|ml|mm|mn|mo|mp|mq|mr|ms|mt|mu|museum|mv|mw|mx|my|mz|na|name|nc|ne|net|nf|ng|ni|nl|no|np|nr|nt|nu|nz|om|org|pa|pe|pf|pg|ph|pk|pl|pm|pn|pr|pro|ps|pt|pw|py|qa|re|ro|ru|rw|sa|sb|sc|sd|se|sg|sh|si|sj|sk|sl|sm|sn|so|sr|st|su|sv|sy|sz|tc|td|tf|tg|th|tj|tk|tm|tn|to|tp|tr|tt|tv|tw|tz|ua|ug|uk|um|us|uy|uz|va|vc|ve|vg|vi|vn|vu|wf|ws|ye|yt|yu|za|zm|zw)$|(([0-9][0-9]?|[0-1][0-9][0-9]|[2][0-4][0-9]|[2][5][0-5])\.){3}([0-9][0-9]?|[0-1][0-9][0-9]|[2][0-4][0-9]|[2][5][0-5]))$/i",$email));
}

if (!defined("PHP_EOL")) define("PHP_EOL", "\r\n");

$name       = $_POST['name'];
$email      = $_POST['email']; 
$message    = $_POST['message'];
$phone      = $_POST['phone'];
$verify     = $_POST['verify'];
$locale			= $_POST['locale'];

if(trim($name) == '') {
	if ($locale == 'en') {
		echo '<div class="error_message">Name has to be filled.</div>';
	} else if ($locale == 'fr') {
		echo '<div class="error_message">Le nom est obligatoire.</div>';
	} else {
		echo '<div class="error_message">El nombre es obligatorio.</div>';
	}
	exit();
} else if(trim($email) == '') {
	if ($locale == 'en') {
		echo '<div class="error_message">Email has to be filled.</div>';
	} else if ($locale == 'fr') {
		echo '<div class="error_message">L\'adresse email est obligatoire.</div>';
	} else {
		echo '<div class="error_message">El correo no es obligatorio.</div>';
	}
	exit();
} else if(!isEmail($email)) {
	if ($locale == 'en') {
		echo '<div class="error_message">Email not valid.</div>';
	} else if ($locale == 'fr') {
		echo '<div class="error_message">Email non valide.</div>';
	} else {
		echo '<div class="error_message">El correo no es válido.</div>';
	}
	exit();
}
if(trim($message) == '') {
	if ($locale == 'en') {
		echo '<div class="error_message">The message cannot be empty.</div>';
	} else if ($locale == 'fr') {
		echo '<div class="error_message">Le message ne peut pas être vide.</div>';
	} else {
		echo '<div class="error_message">El mensaje no puede estar vacío.</div>';
	}
	exit();
}

if(get_magic_quotes_gpc()) {
	$message = stripslashes($message);
}

// Configuration option.

// Email content
$e_subject = '[ReservaZafra.com] Nuevo mensaje desde la pagina web';
$e_content = "$message" . PHP_EOL . PHP_EOL;
$e_details = "\n Nombre: $name \n Tel: $phone \n Correo: $email" . PHP_EOL . PHP_EOL;
$e_reply = "Puede responder directamente a este correo electrónico para responder a $name." . PHP_EOL . PHP_EOL;

$msg = wordwrap( $e_content . $e_details . $e_reply, 70 );

$headers = "From: $name" . PHP_EOL;
$headers .= "Reply-To: $email" . PHP_EOL;
$headers .= "MIME-Version: 1.0" . PHP_EOL;
$headers .= "Content-type: text/plain; charset=utf-8" . PHP_EOL;
$headers .= "Content-Transfer-Encoding: quoted-printable" . PHP_EOL;

if(mail($address, $e_subject, $msg, $headers)) {
	// Email has sent successfully, echo a success page.
	echo "<div id='success_page'>";
	
	if ($locale == 'en') {
		echo "<h4 class='highlight'>Thank you <strong>$name</strong>, your message was successfully sent</h4>";
	} else if ($locale == 'fr') {
		echo "<h4 class='highlight'>Merci <strong>$name</strong>, votre message nous a été envoyé.</h4>";
	} else {
		echo "<h4 class='highlight'>Gracias <strong>$name</strong>, su mensaje ha sido enviado a nosotros.</h4>";
	}

	echo "</div>";
} else {
	echo 'ERROR!';
}